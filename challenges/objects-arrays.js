...
const universities = [];
graduates.map((school) => universities.push(school.university));
universities.sort(); //puts in alphabetical order
...
const contactInfo = [];
graduates.map((person) => contactInfo.push(`${person.first_name} can be contacted at ${person.email}`));
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter((name) => name.university.includes('Uni'));
...
*/
const populationTotal = zooAnimals.reduce((accumalator, animal) => accumalator + animal.population, 0);
...
ALL FUNCTIONS ON PAGE TURNED INTO ARROW FUNCTIONS!
